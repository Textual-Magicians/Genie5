                            ...
                            if (Conversions.ToString(oArgs[0]).Length > 0)
                            {
                                ...
                                switch (switchExpr)
                                {
                                    ...
                                    case "image":
                                    {
                                        ...
                                        foreach (string arg in oArgs)
                                        {
                                            if (arg.StartsWith("#"))
                                                ...
                                                sOutputWindow = arg.Substring(1);
                                            else if ((arg.Length > 2 && arg.StartsWith("w:") || (arg.Length > 6 && arg.StartsWith("width:"))))
                                            {
                                                if (!int.TryParse(arg.Split(":")[1], out width))
                                                    EchoText($"Invalid Width Specified: {arg}");
                                            }
                                            else if ((arg.Length > 2 && arg.StartsWith("h:") || (arg.Length > 7 && arg.StartsWith("height:"))))
                                            {
                                                if (!int.TryParse(arg.Split(":")[1], out height))
                                                    EchoText($"Invalid Height Specified: {arg}");
                                            }
                                            else
                                                filename = arg;
                                        }
                                        if (string.IsNullOrEmpty(filename))
                                            EchoText("No File Name was specified for the Image Command.");
                                        else
                                            DisplayImage(filename, sOutputWindow, width, height);
                                        break;
                                    }
                                    ...
                                    case "lichconnect":
                                    {
                                        ...
                                        if (!File.Exists(oGlobals.Config.CmdPath))
                                            failure += "CMD not found at Path:\t" + oGlobals.Config.CmdPath + System.Environment.NewLine;
                                        if (!File.Exists(oGlobals.Config.RubyPath))
                                            failure += "Ruby not found at Path:\t" + oGlobals.Config.RubyPath + System.Environment.NewLine;
                                        if (!File.Exists(oGlobals.Config.LichPath))
                                            failure += "Lich not found at Path:\t" + oGlobals.Config.LichPath + System.Environment.NewLine;
                                        ...
                                        if (oArgs.Count > 1)
                                        {
                                            ...
                                            switch (switchExpr18)
                                            {
                                                ...
                                                case "position":
                                                {
                                                    ...
                                                    {
                                                        if (oArgs.Count < 4)
                                                            throw new Exception("Syntax Error: Window Name, and one of (Width, Height, Top, or Left) are required.");
                                                        if (oArgs.Count > 7)
                                                            throw new Exception("Error in command: " + sRow + ": " + (oArgs.Count - 7) + " too many Args in Position command." + Interaction.IIf((!int.TryParse(oArgs[3].ToString(), out _)), " Window names with spaces need to be \"enclosed\" in double quotes", ""));
                                                        int? sWidth = null;
                                                        int? sHeight = null;
                                                        ...
                                                        int? sTop = null;
                                                        int? sLeft = null;
                                                        if (oArgs.Count > 5 && int.TryParse(oArgs[5].ToString(), out int top))
                                                            sTop = top;
                                                        if (oArgs.Count > 6 && int.TryParse(oArgs[6].ToString(), out int left))
                                                            sLeft = left;
